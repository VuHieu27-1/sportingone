  ...
  return (
    <form onSubmit={handleLoginSubmit} className="space-y-5">
      <div className="space-y-1 text-left">
        <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-white">Đăng nhập tài khoản</h2>
        <p className="text-xs text-slate-300">Chào mừng trở lại! Vui lòng nhập thông tin của bạn.</p>
      </div>

      <div className="space-y-4 text-left">
        <div>
          <label className="block text-xs font-semibold text-slate-200 mb-1">Tên đăng nhập (Username)</label>
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="rememberMe" className="text-xs text-slate-200 cursor-pointer">
            Ghi nhớ đăng nhập
          </label>
        </div>
        ...
        >
          {isLoading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <span>ĐĂNG NHẬP NGAY</span>}
        </button>
        ...
        <div className="relative my-3">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-[#071711] px-3 text-slate-300 font-medium">Hoặc đăng nhập bằng</span>
          </div>
        </div>
        ...
        <button
          ...
          tabIndex={5}
          onClick={() => {
            const isAddAccount = typeof window !== 'undefined' && window.location.pathname.includes('/add-account');
            authService.loginWithGoogle(isAddAccount);
          }}
          className="w-full py-3 px-4 rounded-xl bg-white hover:bg-slate-100 text-slate-800 font-bold text-sm shadow-md hover:shadow-lg transition-all cursor-pointer flex items-center justify-center gap-3 border border-slate-200 active:scale-[0.99]"
        >
