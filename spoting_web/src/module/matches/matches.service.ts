...
export class MatchsService {
  ...
  async create(dto: CreateMatchDto) {
    const { tournamentId, groupId, yardId, team1Id, team2Id, winnerId, ...cleanDto } = dto;
    ...
    entity.yard = yardVal;
    const team1Val = await this.team1Repository.findOne({ where: { id: dto.team1Id } });
    if (!team1Val) {
      throw new NotFoundException(`Team with ID ${dto.team1Id} not found`);
    }
    entity.team1 = team1Val;
    const team2Val = await this.team2Repository.findOne({ where: { id: dto.team2Id } });
    if (!team2Val) {
    ...
    if (dto.winnerId !== undefined && dto.winnerId !== null) {
      const winnerVal = await this.winnerRepository.findOne({ where: { id: dto.winnerId } });
      if (!winnerVal) {
  ...
  async findAll() {
    return this.repository.find({
      relations: { tournament: true, group: true, yard: true, team1: true, team2: true, winner: true }
    });
  }
