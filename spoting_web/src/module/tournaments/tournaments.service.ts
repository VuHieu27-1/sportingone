...
export class TournamentsService {
  constructor(
    @InjectRepository(Tournament) private readonly repository: Repository<Tournament>,
  ...
  async findAll() {
    return this.repository.find({
      relations: { sportType: true, groups: true, tournamentTeams: true, vendor: { yards: true } }
    });
  }
  ...
  async findOne(id: number) {
    const entity = await this.repository.findOne({
      where: { id },
      relations: { sportType: true, vendor: true, groups: true, tournamentTeams: true }
    });
    if (!entity) {
